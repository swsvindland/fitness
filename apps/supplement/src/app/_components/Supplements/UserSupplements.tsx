...
export const UserSupplements: FC = async () => {
    const userSupplementsQuery =
        await api.supplements.getUserSupplements.query();

    const morningSups = userSupplementsQuery?.filter((item) =>
        item.Times.includes('Morning')
    );
    const breakfastSups = userSupplementsQuery?.filter((item) =>
        item.Times.includes('Breakfast')
    );
    const lunchSups = userSupplementsQuery?.filter((item) =>
        item.Times.includes('Lunch')
    );
    const preWorkoutSups = userSupplementsQuery?.filter((item) =>
        item.Times.includes('PreWorkout')
    );
    const postWorkoutSups = userSupplementsQuery?.filter((item) =>
        item.Times.includes('PostWorkout')
    );
    const dinnerSups = userSupplementsQuery?.filter((item) =>
        item.Times.includes('Dinner')
    );
    const eveningSups = userSupplementsQuery?.filter((item) =>
