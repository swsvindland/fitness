...
export const EditWorkoutExercise: FC<IProps> = ({ index, workoutExercise }) => {
  ...
  useMemo(() => {
    const newOptions = exerciseQuery.data?.data.map((item) => item.name) || [];
  ...
  const handleSubmit = () => {
    ...
    mutation.mutate({
      ...
      exerciseId:
        exerciseQuery.data?.data.filter((item) => item.name === option).at(0)
          ?.id ?? 0,
      sets: parseInt(sets),
  ...
  return (
    <div className="card my-2 p-4">
      {!saved && <span className="mb-4 text-ternary">*Unsaved changes</span>}
      <Autocomplete
        label="Exercise"
        isLoading={false}
        selected={option}
      ...
      <span className="my-4 text-ternary">
        *Choose either a time or rep range. If you choose a time and rep range,
        it will only show a time based set.
      </span>
      <div className="flex flex-row">
        <TextField
          label="MinReps"
